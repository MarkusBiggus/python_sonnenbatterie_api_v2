...
async def test_coroutine_methods(battery_charging: sonnenbatterie) -> None:
    ...
    def _test_get_status():
        ...
        return battery_charging.get_status()

    def _test_get_systemdata():
        """Coroutine to sync fetch"""
        return battery_charging.get_systemdata()

    def _test_get_latest_data():
        """Coroutine to sync fetch"""
        return battery_charging.get_latest_data()

    def _test_get_configurations():
        """Coroutine to sync fetch"""
        return battery_charging.get_configurations()

    def _test_get_battery():
        """Coroutine to sync fetch"""
        return battery_charging.get_battery()

    def _test_get_batterysystem():
        """Coroutine to sync fetch"""
        return battery_charging.get_batterysystem()

    def _test_get_powermeter():
        """Coroutine to sync fetch"""
        return battery_charging.get_powermeter()

    def _test_get_inverter():
        """Coroutine to sync fetch"""
        return battery_charging.get_inverter()
    ...
    assert battery_charging.get_login_timeout() == 120
    assert battery_charging.set_login_timeout(25) == 25
    assert battery_charging.get_request_connect_timeout() == 60
    assert battery_charging.set_request_connect_timeout(25) == 25
    assert battery_charging.get_request_read_timeout() == 60
    assert battery_charging.set_request_read_timeout(15) == 15

    assert battery_charging.get_current_charge_level() == 88
    assert battery_charging.get_operating_mode() == 2
    assert battery_charging.get_operating_mode_name() == 'Automatic - Self Consumption'
